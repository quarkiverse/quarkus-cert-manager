...
Apart from these two resources, the Quarkus Cert-Manager extension will also configure, part of the Deployment, a volume mounted from the secret that contains the Cert-Manager generated files to allow the application to access them and to configure the HTTPS/TLS endpoint:
...
Furthermore, by default it will autoconfigure the Ingress resource if the property `quarkus.kubernetes.ingress.expose` is true:
...
[IMPORTANT]
====
The autoconfiguration of the Ingress TLS and HTTP SSL configurations can be disabled using the property `quarkus.certificate.http-ssl-auto-configuration=false`.
====
...
In Quarkus, you can generate the above Ingress resource by simply adding the following key properties:
[source,properties]
----
quarkus.kubernetes.ingress.host=foo.bar.com
quarkus.kubernetes.ingress.expose=true
quarkus.kubernetes.ingress.tls-secret-name=tls-secret
----
